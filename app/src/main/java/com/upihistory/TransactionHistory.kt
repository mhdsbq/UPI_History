...
fun TransactionHistory(transactions: List<UpiTransaction>) {
    ...
    LazyColumn(
        ...
        grouped.forEach { (monthYear, txns) ->
            ...
            item {
                Box(
                    ...
                    Row(
                        ...
                        Text(
                            text = "₹${"%.0f".format(totalMonthlySpend)}",
            ...
            txns.forEach { txn ->
                val currentDateLabel = formatDateLabel(txn.timestamp.toLong() * 1000)
...
fun TransactionItem(transaction: UpiTransaction) {
    val date = formatDateTime(transaction.timestamp.toLong() * 1000)
    val nameOrVpa = transaction.receiverName?.takeIf { it.isNotBlank() }
        ?: transaction.receiverVpa.orEmpty()
    ...
    Box(
        ...
        Row(
            ...
            Text(
                text = "₹${"%.0f".format(transaction.amount / 100.0)}",
                color = MaterialTheme.colorScheme.onSurfaceVariant,
                style = MaterialTheme.typography.bodyLarge
            )
        }
...
fun groupTransactionsByMonth(transactions: List<UpiTransaction>): Map<String, List<UpiTransaction>> {
    ...
    return transactions.sortedByDescending { it.timestamp }
        .groupBy { sdf.format(Date(it.timestamp.toLong() * 1000)) }
}
